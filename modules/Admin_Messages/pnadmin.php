<?php
// $Id: pnadmin.php 17509 2006-01-12 12:20:17Z markwest $
// ----------------------------------------------------------------------
...
// Purpose of file:  Admin Messages administration display functions
// ----------------------------------------------------------------------
...
function Admin_Messages_admin_view()
{
    ...
    // obtained from the input and gets us the information on the appropriate
    // items.
    $items = pnModAPIFunc('Admin_Messages',
                          'user',
                          'getall',
                          array('startnum' => $startnum,
                                'numitems' => pnModGetVar('Admin_Messages',
                                                          'itemsperpage')));

    $rows = array();
    foreach ($items as $item) {

        if (pnSecAuthAction(0, 'Admin_Messages::', "$item[title]::$item[mid]", ACCESS_READ)) {

            $fullitem = pnModAPIFunc('Admin_Messages',
                                     'user',
                                     'get',
                                      array('mid' => $item['mid']));
            ...
            switch ($fullitem['whocanview']) {
                case '1':
                    $fullitem['whocanview'] = _ADMINMESSAGESALL;
                    break;
                case '2':
                    $fullitem['whocanview'] = _ADMINMESSAGESUSERS;
                    break;
                case '3':
                    $fullitem['whocanview'] = _ADMINMESSAGESANON;
                    break;
                case '4':
                    $fullitem['whocanview'] = _ADMINMESSAGESADMIN;
                    break;
            }
            $row[] = $fullitem['whocanview'];
            ...
            if ($fullitem['expire'] == 0) {
			    $expire = _ADMINMESSAGESUNLIMITED;
			} else if ($fullitem['expire']/86400 == 1) {
                $expire = $fullitem['expire']/86400 . ' ' . _ADMINMESSAGESDAY;
			} else {
                $expire = $fullitem['expire']/86400 . ' ' . _ADMINMESSAGESDAYS;
            }

            if ($fullitem['expire'] == 0) {
			    $expiredate = _ADMINMESSAGESNOEXPIRE;
			} else {
                $expiredate = ml_ftime(_DATETIMEBRIEF, $fullitem['date'] + $fullitem['expire']);
			}
            ...
            // for display
            $options = array();
            if (pnSecAuthAction(0, 'Admin_Messages::', "$item[title]::$item[mid]", ACCESS_EDIT)) {
                $options[] = array('url' => pnModURL('Admin_Messages', 'admin', 'modify', array('mid' => $item['mid'])),
                                   'text' => _EDIT);
                if (pnSecAuthAction(0, 'Admin_Messages::', "$item[title]::$item[mid]", ACCESS_DELETE)) {
			...
			$rows[] = array( 'mid' => $item['mid'],
							 ...
							 'whocanview' => $fullitem['whocanview'],
							 'active' => $active,
							 'expire' => $expire,
